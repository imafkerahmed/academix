...
export function RegisterStudentModal({
  ...
  const getMissingRequiredFields = () => {
    ...
    if (
      ...
      enrollNow &&
      formData.paymentOption === "upfront_installments" &&
      formData.customUpfrontAmount <= 0
    ) {
      missing.push("Upfront Amount (must be greater than 0)");
    }
      ...
      if (enrollNow && formData.courseIntakeFeeId) {
        ...
        if (selectedFee && selectedIntake) {
          ...
          try {
            ...
            if (
              formData.includeRegistrationFee &&
              feeCalc.registration_fee_amount > 0
            ) {
              await pb.collection("payments").create({
                ...
                student: studentId,
                amount: feeCalc.registration_fee_amount,
            ...
            const coursePortion =
              feeCalc.upfront_payment -
              (formData.includeRegistrationFee
                ? feeCalc.registration_fee_amount
                : 0);
            if (coursePortion > 0) {
              const coursePayType =
      ...
      <div className="space-y-5">
              ...
              {[1, 2, 3, 4].map((s) => (
                <div
                  ...
                  className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${
                    stage >= s ? "bg-indigo-600" : "bg-gray-100"
                  }`}
                />
              ))}
            </div>
        ...
        {stage === 1 && (
          <div className="space-y-5 animate-in fade-in duration-300">
            ...
            <div className="pt-2 border-t border-gray-50">
              <p className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-4 ml-1">
                Guardian Details
              </p>
                ...
                <div className="relative">
                  ...
                  {avatarPreview && (
                    <button
                      ...
                      className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-lg flex items-center justify-center shadow border-2 border-white"
                    >
                      <X size={12} />
                    </button>
                  )}
                </div>
                <label className="cursor-pointer flex items-center gap-2 px-5 py-3 bg-gray-50 border border-gray-100 hover:bg-gray-100 text-gray-700 rounded-2xl transition-all font-bold text-xs uppercase tracking-widest">
                  <Upload size={14} />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <Badge className="absolute right-3 top-1/2 -translate-y-1/2 bg-indigo-600 text-[9px] font-black text-white px-2 py-1">
                    Auto-Generated
                  </Badge>
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 font-bold transition-all text-gray-900"
                  />
                  <Badge className="absolute right-3 top-1/2 -translate-y-1/2 bg-gray-100 text-[9px] font-black text-gray-400 px-2 py-1">
                    Optional
                  </Badge>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {["Colombo", "Negambo"].map((branch) => (
                  <button
                    ...
                    className={`px-4 py-4 rounded-2xl text-xs font-black tracking-widest border-2 transition-all ${
                      formData.branch === branch
                        ? "border-indigo-600 bg-indigo-50 text-indigo-700"
                        : "border-gray-100 bg-gray-50 text-gray-400 hover:border-indigo-100"
                    }`}
                  >
                    {branch.toUpperCase()}
                  </button>
            ...
            <div className="p-1.5 bg-gray-50 rounded-[2rem] border border-gray-100 mb-2">
              <button
                ...
                className={`w-full flex items-center justify-between p-4 rounded-[1.6rem] transition-all duration-500 ${
                  enrollNow ? "bg-white shadow-sm" : "bg-transparent opacity-60"
                }`}
              >
                    ...
                    {formData.paymentOption === "upfront_installments" && (
                      <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                        ...
                        <input
                          ...
                          step="100"
                          value={formData.customUpfrontAmount || ""}
                          onChange={(e) => {
                            const val = parseFloat(e.target.value) || 0;
                            setFormData((prev) => ({
                              ...prev,
                              customUpfrontAmount: val,
                            }));
                          }}
                          placeholder="e.g. 5000"
                          className="w-full px-5 py-4 bg-white border-2 border-indigo-100 rounded-2xl text-sm font-bold text-gray-900 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all placeholder:text-gray-300"
                        />
                        {feeCalculation && (
                    ...
                    {feeCalculation && (
                      <div className="space-y-3 p-6 bg-white rounded-2xl border-2 border-indigo-200 shadow-lg">
                        ...
                        <div className="pt-4 border-t-2 border-indigo-100">
                          {formData.paymentOption === "installments_only" ? (
        ...
        <div className="flex flex-col gap-3 pt-6 border-t border-gray-50">
          ...
          <button
            ...
            className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {loading ? (
              ...
              <>
                <Check size={16} />
                {enrollNow ? "Complete Enrollment" : "Register Pending Student"}
              </>
            ) : (
              <>
                Continue
