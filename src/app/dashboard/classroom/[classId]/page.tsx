...
export default function VirtualClassroom() {
  ...
  const handleJoin = async () => {
    ...
    const isHost = (isAdmin && requestedRole === "host") || isOwner;
    const password = isHost ? "lecturer123" : "student123";
    ...
    setIsJoining(false);
    // If connect succeeded, error should be cleared
    setLocalError(null);
  };
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-indigo-400 font-black uppercase tracking-widest">
                    {classData.expand?.course_subject?.expand?.subject?.[0]
                      ?.name ||
                  ...
                  <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                    {
                      classData.expand?.course_subject?.expand?.course_intake
                        ?.expand?.course?.name
                    }
                  </span>
                </div>
          ...
          <div className="pointer-events-auto flex items-center gap-2">
            <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-xl flex items-center gap-3 border border-white/10">
              ...
              <span className="text-xs font-black tracking-widest">
                {remoteStreams.length + (connected ? 1 : 0)} PARTICIPANTS
              </span>
            </div>
          </div>
        </div>
              ...
              <div className="relative group bg-gray-900 rounded-[2rem] overflow-hidden border border-white/5 shadow-2xl">
                ...
                <div className="absolute bottom-6 left-6 z-10 bg-black/40 backdrop-blur-md px-4 py-1.5 rounded-xl border border-white/10 flex items-center gap-2">
                  <span className="text-[10px] font-black tracking-widest uppercase">
                    ...
                    classData?.expand?.course_subject?.lecturer?.includes(
                      currentUser?.id,
                    )
                      ? "Teacher"
                      : "Student"}
        ...
        <div className="w-96 bg-gray-900/50 backdrop-blur-3xl border-l border-white/5 flex flex-col animate-in slide-in-from-right duration-300 shadow-2xl">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar">
            {chatMessages.map((msg, i) => (
              <div
                key={i}
                className={`flex flex-col ${msg.username === (currentUser?.name || currentUser?.username) ? "items-end" : "items-start"}`}
              >
                ...
                <div
                  className={`px-4 py-3 rounded-2xl max-w-[85%] text-sm font-medium leading-relaxed ${
                    msg.username ===
          ...
          <form
            ...
            className="p-6 bg-black/20 backdrop-blur-2xl border-t border-white/5"
          >
